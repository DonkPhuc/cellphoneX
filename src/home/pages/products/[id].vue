  ...
  <VDialog :is-open="open" @close="open = false">
    <template #title>
      <div class="flex items-center gap-2">
        <img class="h-10 w-10" src="https://cdn2.cellphones.com.vn/213x213,webp,q100/media/wysiwyg/Shipper_CPS.jpg"
          alt="" />
    ...
    <template #action>
      <div class="mb-5 flex justify-between gap-2">
        ...
        <VButton input-class="w-36 ring-0" size="large" variant="outlined" class="w-20" label="Để sau"
          @click="open = false" />
      </div>
    </template>
  </VDialog>
          ...
          <div class="flex-1">
            <div class="flex flex-col gap-4">
              ...
              <div class="flex gap-4">
                <div v-for="item in listRom" :key="item.value"
                  :class="selectedRom === item.value ? 'border border-main' : ''"
                  class="h-20 flex-1 cursor-pointer rounded-md border" @click="selectedOptionRom(item.value)">
              ...
              <div class="flex gap-4">
                <div v-for="item in listColor" :key="item.value"
                  :class="selectedColor === item.value ? 'border border-main' : ''"
                  class="h-10 flex-1 cursor-pointer rounded-md border" @click="selectedOptionColor(item.value)">
              ...
              <div class="flex gap-2">
                <div class="flex-1 rounded-xl">
                  <VButton input-class="w-full !h-14 rounded-xl border-none !bg-main" :style="'2line'" label="Mua Ngay"
                    label2="(Giao hàng nhanh từ 2 giờ hoặc nhận tại cửa hàng)" @click="addToCart('buy')" />
                </div>
                <div class="flex-[1/4] rounded-xl">
                  <VButton :style="'2line'"
                    ...
                    variant="outlined" label2="Thêm vào giỏ" icon="fa-cart-arrow-down" @click="addToCart('cart')" />
                </div>
              </div>
              ...
              <div class="flex gap-2">
                <div class="flex-1 rounded-xl">
                  <VButton :disabled="true"
                    ...
                    label="trả góp 0%" :style="'2line'" label2="(Xét duyệt qua điện thoại)" />
                </div>
                <div class="flex-1 rounded-xl">
                  <VButton :disabled="true"
                    input-class="disabled:!text-white w-full !h-14 rounded-xl border-none !bg-[linear-gradient(180deg,#3a78d0,#277cea)]"
                    :style="'2line'" label2="(Visa, MasterCard, JCB)" label="trả góp qua thẻ" />
                </div>
              </div>
            </div>
          </div>
