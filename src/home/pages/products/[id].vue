...
async function getData() {
  currentProduct.value = (await store.getProduct(
    route.params.id.toString()
  )) as Products;
  currentProduct.value.price =
    currentProduct.value.priceRRP -
    (currentProduct.value.priceRRP * currentProduct.value.discount) / 100;
  ...
  currentProduct.value.rate.forEach((element: { value: number }) => {
    averageRate.value =
      (averageRate.value + element.value) / currentProduct.value.rate.length;
  });
...
async function addToCart(action: string) {
  if (!isLoginSuccess.value) {
    ...
    const rs = await store.postAddToCart(
...
function selectedOptionRom(value: number) {
  ...
  currentProduct.value.price =
    currentPrice.value * selectedColor.value * selectedRom.value;
  currentProduct.value.priceRRP =
    currentPrice.value * selectedColor.value * selectedRom.value;
}
...
async function postRating() {
  let result;
  let rs = currentProduct.value.rate.find(
...
<template>
  <VDialog :is-open="open" @close="open = false">
    ...
    <template #detail>
      <p class="w-[350px] text-justify font-bold">
    ...
    <template #title>
      <div class="flex flex-col items-center gap-2">
        <img
          ...
          alt=""
        />
        <span class="text-center font-bold md:px-8 md:text-2xl"
          >Đánh giá & nhận xét {{ currentProduct.name }}</span
        >
      </div>
    </template>
    <template #detail>
      ...
      <div class="flex w-full flex-col gap-4 rounded-xl border p-4">
        <p class="text-justify text-xs font-bold md:text-lg">
        ...
        <div class="text-md flex justify-center gap-2">
          ...
          >
            <VIcon
              :icon-class="
                ratingProduct >= index ? 'text-yellow-600' : 'text-black-600'
              "
              icon="fa-star"
            />
        ...
        <div class="flex w-full items-center gap-4 border-b-2 px-4 py-2">
          ...
          <div class="hidden pl-8 lg:flex">
            <div class="hidden items-center justify-end lg:flex">
              <VIcon
                :icon-class="averageRate > 0 ? 'text-yellow-600' : 'text-black'"
                icon="fa-star"
              />
              <VIcon
                :icon-class="averageRate > 1 ? 'text-yellow-600' : 'text-black'"
                icon="fa-star"
              />
              <VIcon
                :icon-class="averageRate > 2 ? 'text-yellow-600' : 'text-black'"
                icon="fa-star"
              />
              <VIcon
                :icon-class="averageRate > 3 ? 'text-yellow-600' : 'text-black'"
                icon="fa-star"
              />
              <VIcon
                :icon-class="averageRate > 4 ? 'text-yellow-600' : 'text-black'"
                icon="fa-star"
              />
            </div>
            <span class="hidden lg:flex"
              >{{ currentProduct.rate.length + " " }} đánh giá</span
            >
          </div>
        </div>

        <div
          class="flex flex-col gap-4 border-b-2 p-4 md:flex-row md:px-2 lg:flex-row"
        >
          <div class="flex-1">
            ...
            <div class="flex flex-col gap-4">
              ...
              <div class="flex gap-4">
                <div
                  ...
                  :key="item.value"
                  :class="
                    selectedRom === item.value ? 'border border-main' : ''
                  "
                  class="h-20 flex-1 cursor-pointer rounded-md border"
                  @click="selectedOptionRom(item.value)"
                >
                  <span
                    class="flex h-full items-center justify-center font-bold"
                  >
                    {{ item.rom }}</span
                  >
                </div>
              ...
              <div class="flex gap-4">
                <div
                  ...
                  :key="item.value"
                  :class="
                    selectedColor === item.value ? 'border border-main' : ''
                  "
              ...
              <div class="flex flex-col">
                <div
                  class="rounded-t-2xl bg-[#FEE2E2] p-2 !font-bold text-main"
                >
                  <VIconLabel
                  ...
                  <div class="flex flex-col">
                    <div class="flex items-center gap-2 p-2">
                      <p class="max-h-8 text-xs">
                        Giảm ngay 3.000.000đ khi tham gia thu cũ đổi mới - Giá
                        thu tốt nhất thị trường (Xem chi tiết)
                      </p>
                    </div>
                    <div class="flex items-center gap-2 p-2">
                      <p class="max-h-8 text-xs">
              ...
              <div class="flex flex-col gap-4 rounded-xl border p-4">
                ...
                <span class="flex items-center gap-2">
                  ...
                  <p class="text-sm font-extralight">
                    Nguyên hộp, đầy đủ phụ kiện từ nhà sản suất<br />Bảo hành
                    pin 12 tháng
                  </p>
                </span>
                <span class="flex items-center gap-2">
              ...
              <div class="flex flex-col gap-4 rounded-xl border p-4">
                <span class="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p class="text-sm">
                      (Khách hàng đăng ký thông tin để được hỗ trợ tư vấn và
                      thanh toán tại cửa hàng nhanh nhất, số tiền phải thanh
                      toán chưa bao gồm giá trị của gói bảo hành mở rộng)
                    </p>
                  </div>
                </span>
              </div>
        ...
        <div class="flex flex-col gap-4 md:flex-row lg:flex-row">
          <div class="flex-1 rounded-xl border p-4 shadow-xl">
            <div class="flex flex-col gap-4 rounded-xl">
              ...
              <div class="flex">
                <div
                  class="flex flex-1 items-center justify-center border md:flex-[0.7] lg:rounded-l-2xl"
                >
                  <div class="flex flex-col items-center">
                    <span class="flex justify-center text-2xl font-bold"
                      >{{ averageRate.toFixed(1) }}/5</span
                    >
                    <div class="flex items-center">
                      <VIcon
                        :icon-class="
                          averageRate > 0 ? 'text-yellow-600' : 'text-black-600'
                        "
                        icon="fa-star"
                      />
                      <VIcon
                        :icon-class="
                          averageRate > 1 ? 'text-yellow-600' : 'text-black-600'
                        "
                        icon="fa-star"
                      />
                      <VIcon
                        :icon-class="
                          averageRate > 2 ? 'text-yellow-600' : 'text-black-600'
                        "
                        icon="fa-star"
                      />
                      <VIcon
                        :icon-class="
                          averageRate > 3 ? 'text-yellow-600' : 'text-black-600'
                        "
                        icon="fa-star"
                      />
                      <VIcon
                        :icon-class="
                          averageRate > 4 ? 'text-yellow-600' : 'text-black-600'
                        "
                        icon="fa-star"
                      />
                    </div>
                    <span class="flex justify-center">
                      <b> {{ currentProduct.rate.length + " " }}</b> đánh giá và
                      nhận xét
                    </span>
                  </div>
                </div>
                <div class="hidden flex-1 rounded-r-2xl border p-2 md:block">
                  <div class="flex items-center">
                    ...
                    ></div>
                    <p class="flex flex-[0.4] justify-center">
                      {{ fiveStar }} đánh giá
                    </p>
                  </div>
                  <div class="flex items-center">
              ...
              >
                <div
                  v-for="item in currentProduct.rate"
                  :key="item.username"
                  class="flex-col"
                >
                  <div class="flex justify-between">
                    <div class="flex gap-2">
                      <span
                        class="flex h-5 w-5 items-center justify-center bg-red-100 font-bold"
                      >
                  ...
                  <div class="flex justify-end">
                    <div
                      class="flex flex-1 flex-col gap-2 rounded-xl bg-[#f3f4f6] p-2"
                    >
                      <div class="flex">
              ...
              <div class="flex flex-col gap-2 p-4">
                ...
                <div class="flex flex-col rounded-xl border">
                  ...
                  <div class="flex justify-between bg-[#f2f2f2] p-2">
                    ...
                    <p class="flex flex-1 items-center text-sm">
                      DDR4 8GB (1 x 8GB) 3200MHz, up to 32GB
                    </p>
                  </div>
                  <div class="flex justify-between p-2">
